...
namespace graphgen {
    ...
    std::shared_ptr<Graph> generateConnectedGraph2(int num_nodes, float density,
        ...
        int edges_remaining = num_edges - num_nodes;
        std::vector<int> edges_count(num_nodes, 1);

        std::vector<bool> connected(num_nodes, false);
        std::vector<int> connected_nodes, disconnected_nodes;
        std::iota(disconnected_nodes.begin(), disconnected_nodes.end(), 0);

        srand(seed);

        // distribute how many edges each node has randomly
        if (edges_remaining <= num_nodes)
        {
            while(edges_remaining < 0)
            {
                int random_node = rand() % num_nodes;

                if (edges_count[random_node] > 0)
                {
                    edges_remaining++;
        ...
        connected_nodes.push_back(random_node);
        disconnected_nodes.erase(std::remove(disconnected_nodes.begin(), disconnected_nodes.end(), random_node), disconnected_nodes.end());

        int connected_count = 0;
        while (connected_count < num_nodes && connected_count < num_edges)
        {
