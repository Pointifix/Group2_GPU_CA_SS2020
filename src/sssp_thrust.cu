...
{
    thrust::host_vector<pos_t> previous_nodes(graph->edges.size(), -1);
    thrust::host_vector<mask_t> mask(graph->edges.size(), 0);
    thrust::host_vector<weight_t> cost(graph->edges.size(), std::numeric_limits<int>::max());

    mask[source_node] = true;
    cost[source_node] = 0;

    thrust::device_vector<pos_t> d_edges = graph->edges;
    thrust::device_vector<pos_t> d_destinations = graph->destinations;
    thrust::device_vector<weight_t> d_weights = graph->weights;
    thrust::device_vector<pos_t> d_previous_node = previous_nodes;
    thrust::device_vector<mask_t> d_mask = mask;
    thrust::device_vector<weight_t> d_cost = cost;
    ...
    while (std::find(mask.begin(), mask.end(), true) != mask.end())
    {
        ...
        M_CFUN((alg::SSSP_Kernel<<<numBlocks, M_BLOCKSIZE>>>(
    ...
    std::vector<pos_t> ret_previous_nodes(previous_nodes.size());
    thrust::copy(previous_nodes.begin(), previous_nodes.end(), ret_previous_nodes.begin());

    std::vector<weight_t> ret_cost(cost.size());
